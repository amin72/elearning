...
class CourseListView(TemplateResponseMixin, View):
    ...
    def get(self, request, subject=None):
        ...
        # if subjects not cached yet or timed out, cache them
        if not subjects:
            subjects = Subject.objects.annotate(total_courses=Count('courses'))
            cache.set('all_subjects', subjects)

        all_courses = Course.objects.annotate(total_courses=Count('modules'))
        ...
        if subject:
            ...
            if not courses:
                carses = all_courses.filter(subject=subject)
